    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-3.5 flex justify-between items-center relative">
        ...
        <div className="hidden md:flex items-center gap-7">
          ...
          <div className="relative" ref={appsMenuRef}>
            ...
            >
              ...
              <span>{t('nav.apps', {}, 'Tətbiqlər')}</span>
              <span
            ...
            {isAppsOpen && (
              ...
              >
                ...
                <div className="pt-4 flex items-center justify-between text-xs text-on-surface-variant/80">
                  ...
                  >
                    {t('nav.allApps', {}, 'Bütün Tətbiqlərə Keçid')} →
                  </Link>
                </div>
              </div>
            )}
          ...
          <Link
            ...
            className={`font-label-sm text-xs uppercase tracking-wider transition-colors ${
              location.pathname.startsWith('/product') || location.pathname === '/crm' || location.pathname === '/tasks'
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <div className="relative" ref={langMenuRef}>
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all cursor-pointer ${
                isDarkMode
                  ? 'border-white/15 bg-white/5 text-slate-200 hover:text-white hover:bg-white/10 hover:border-white/25'
                  : 'border-slate-200 bg-slate-50/80 text-slate-700 hover:text-slate-900 hover:bg-slate-100'
              }`}
              title={t('navbar.language', {}, 'Dil')}
            >
              <FlagIcon code={language} className="w-4 h-3 rounded-[2px]" />
              <span className="uppercase tracking-wider text-[11px] font-bold">{language}</span>
            </button>

            {isLangOpen && (
              <div
                className={`absolute right-0 mt-2 w-48 rounded-2xl border shadow-2xl p-1.5 z-50 text-xs animate-in fade-in duration-150 backdrop-blur-xl ${
                  isDarkMode ? 'bg-[#1C1C1E]/95 border-[#2C2C2E] text-[#E4E4E7] shadow-[0_15px_35px_rgba(0,0,0,0.6)]' : 'bg-white/95 border-slate-200 text-slate-800 shadow-xl'
                }`}
              >
                {languages.map((l) => (
                  <button
                  ...
                  >
                    ...
                    {language === l.code && <span className="text-sky-400 font-bold text-sm">✓</span>}
                  </button>
                ))}
              </div>
            )}
          ...
          >
            {isUserLoggedIn ? t('nav.workspace', {}, 'Workspace') : t('nav.login', {}, 'Daxil Ol')}
          </Link>
          <Link
            to={startDestination}
      ...
      {isMobileMenuOpen && (
        ...
        >
          <div className={`space-y-3 pb-4 border-b ${isDarkMode ? 'border-white/10' : 'border-slate-100'}`}>
            ...
            >
              <img src={taskManagementLogo} alt="Tasks" className="w-5 h-5 object-contain" /> {t('nav.tasks', {}, 'Task Management')}
            </Link>
            <Link
              to="/crm"
              onClick={() => setIsMobileMenuOpen(false)}
              className="flex items-center gap-3 text-sm"
            >
              <img src={altensorCrmLogo} alt="CRM" className="w-5 h-5 object-contain" /> {t('nav.crm', {}, 'Altensor CRM')}
            </Link>
          </div>

          <div className={`flex flex-col gap-3 pb-4 border-b text-sm ${isDarkMode ? 'border-white/10' : 'border-slate-100'}`}>
            <Link to="/products" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-primary font-medium">
              {t('nav.products', {}, 'Məhsul')}
            </Link>
            <Link to="/about" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-primary font-medium">
              {t('nav.about', {}, 'Haqqımızda')}
            </Link>
          </div>

          <div className={`flex items-center justify-between pb-4 border-b ${isDarkMode ? 'border-white/10' : 'border-slate-100'}`}>
            <span className="text-[11px] font-label-sm uppercase tracking-widest text-primary font-bold">
              {t('navbar.language', {}, 'Dil')}
            </span>
            <div className="flex items-center gap-1.5">
              {languages.map((l) => (
                ...
                >
                  <FlagIcon code={l.code} className="w-3.5 h-2.5 rounded-[2px]" />
